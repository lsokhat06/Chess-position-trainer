...
function App() {
  ...
  function parseThemes() {
    ...
    for (let i = 0; i < PUZZLES.length; ++i) {
      let data_excerpts = PUZZLES[i]["Themes"].split(" ");
      for (let j = 0; j < data_excerpts.length; ++j) {
        themes.add(data_excerpts[j]);
      }
    }
    return themes;
  }
  ...
  function parseOpenings() {
    ...
    for (let i = 0; i < PUZZLES.length; ++i) {
      let data_excerpt = PUZZLES[i]["OpeningFamily"].replace(/_/g, ' ');
      if (data_excerpt != "") { openings.add(data_excerpt); }
    }
    return openings;
  }

  if (!localStorage.hasOwnProperty("themes")) {
    let types = ["themes", "openings"];
    for (let i = 0; i < types.length; ++i) {
      let states = {};
      let data = (types[i] == "themes" ? parseThemes() : parseOpenings());
      for (const item of data) {
        states[item] = false;
      }
      localStorage.setItem(types[i], JSON.stringify(states));
    }
  }
  ...
  if (!(getNormalDate() in tstats)) {
    let today = getNormalDate();
    tstats[today] = {
      puzzlesStarted: 0,
      puzzlesFailed: 0,
      puzzlesSolved: 0,
  ...
  let [currentPuzzle, setCurrentPuzzle] = useState(DEFAULT);
  ...
  let [menuLayer, setMenuLayer] = useState(0);

  let [foundPuzzle, setFoundPuzzle] = useState(false);
  ...
  function update(won) {
    ...
    let today = getNormalDate();
    let t_curr_diff = parseInt(currentPuzzle["Rating"]);

    stats["avgPuzzleRating"] = Math.floor(((stats["avgPuzzleRating"] * stats["puzzlesStarted"]) + t_curr_diff) / (stats["puzzlesStarted"] + 1));
    stats["puzzlesStarted"] += 1;
    if (t_curr_diff > stats["highestRatedPuzzle"]) { 
      stats["highestRatedPuzzle"] = t_curr_diff;
    }
    if (won) { 
    ...
    stats["ratingDynamics"].push(won ? newRatingWins : newRatingLoses);



    stats[today]["avgPuzzleRating"] = Math.floor(((stats[today]["avgPuzzleRating"] * stats[today]["puzzlesStarted"]) + t_curr_diff) / (stats[today]["puzzlesStarted"] + 1));
    stats[today]["puzzlesStarted"] += 1;
    if (t_curr_diff > stats[today]["highestRatedPuzzle"]) { 
      stats[today]["highestRatedPuzzle"] = t_curr_diff;
    }
    if (won) { 
      stats[today]["puzzlesSolved"] += 1; 
    }
    if (!won) {
      stats[today]["puzzlesFailed"] += 1;
    }
    stats[today]["ratingDynamics"].push(won ? newRatingWins : newRatingLoses);
    localStorage.setItem("stats", JSON.stringify(stats));
  }

  function setPuzzle(deepCopy = true) {
    ...
    let localThemes = JSON.parse(localStorage.getItem("themes"));
    let localOpenings = JSON.parse(localStorage.getItem("openings"));

    let preferredThemes = [];
    let preferredOpenings = [];

    for (const [key, value] of Object.entries(localThemes)) {
      if (value) { 
        preferredThemes.push(key);
      }
    }
    for (const [key, value] of Object.entries(localOpenings)) {
      if (value) { 
        preferredOpenings.push(key);
      }
    }

    let includeThemes = (preferredThemes.length > 0);
    let includeOpenings = (preferredOpenings.length > 0);

    let validPuzzles = [];

    for (let i = 0; i < PUZZLES.length; ++i) {
      let temp_themes = PUZZLES[i]["Themes"].split(" ");
      let temp_opening = PUZZLES[i]["OpeningFamily"].replace(/_/g, ' ');

      let with_theme = false;
      let with_opening = false;

      if (includeThemes) {
        for (let j = 0; j < temp_themes.length; ++j) {
          if (preferredThemes.includes(temp_themes[j])) {
            with_theme = true;
            break;
          }
        }
      }
      if (includeOpenings && preferredOpenings.includes(temp_opening)) {
        with_opening = true;
      }

      if (includeThemes == with_theme && includeOpenings == with_opening) {
        PUZZLES[i]["NbPlays"] += "1";
        validPuzzles.push({...PUZZLES[i]});
      }
    }
    if (validPuzzles.length == 0) { setFoundPuzzle(false); }
    else { setFoundPuzzle(true); }
    let t_puzzle = (validPuzzles.length == 0 ? PUZZLES[Math.floor(Math.random() * PUZZLES.length)] : validPuzzles[Math.floor(Math.random() * validPuzzles.length)]);
    if (deepCopy) {
    setCurrentPuzzle({...t_puzzle});
    }

    else { return {...t_puzzle}; }
  ...
  let to_find = params.get("puzzle");
  if (to_find != null && currentPuzzle["PuzzleId"] != to_find) {
    for (let i = 0; i < PUZZLES.length; ++i) {
      if (PUZZLES[i]["PuzzleId"] == to_find) {
        PUZZLES[i]["NbPlays"] += "1";
  ...
  function changePreferenceState(id, type) {
    for (let i = 0; i < 8; ++i) {
      for (let j = 0; j < 8; ++j) {
        document.getElementById(String.fromCharCode(97 + i) + String.fromCharCode(49 + j)).style.border = "none";
      }
    }
    let states = JSON.parse(localStorage.getItem(type));
    states[id] = document.getElementById(id).checked;
    localStorage.setItem(type, JSON.stringify(states));
    setCurrentPuzzle(setPuzzle(false));
  }

  if (currentPuzzle == DEFAULT) { 
    setCurrentPuzzle(setPuzzle(false));
  }
  ...
  return (
    <div className="App">
      ...
      pieceOfPromotion, setPieceOfPromotion, correctPromotion, setCorrectPromotion, moveHintUsed, setMoveHintUsed, menuLayer,
      setMenuLayer, changePreferenceState, getNormalDate}}>
      <Menu/>

      <div class = "fwrapper" style = {{"display": (menuLayer == 0 ? "flex" : "none")}}>

      <Board currentPuzzle = {currentPuzzle} pieceOfPromotion = {pieceOfPromotion}/>
      <div>
        <Info currentPuzzle = {currentPuzzle} myRating = {rating} id = {currentPuzzle["PuzzleId"]} found = {foundPuzzle} />
        <div class = "fwrapper">
          <Hint/>
          <Move/>
        </div>
      ...
      <div style = {{"display": (menuLayer == 1 ? "block" : "none")}}>
      <Statistics/>
      </div>

      <div style = {{"display": (menuLayer == 2 ? "block" : "none")}}>
        <Preferences themes = {parseThemes()} openings = {parseOpenings()}/>
      </div>
