...
function Board(props) {
  ...
  function transformToHTML(json) {
    ...
    for (const [key, value] of Object.entries(json)) {
      squares.push(
        <div class = {"square " + value["color"] + " " + (value["selected"] ? "selected" : "")} id = {value["id"]} onClick = {() => handleClick(value["id"])}>
        {"letter" in value ? <p class = "squareLetter"><b>{value["letter"]}</b></p> : <p></p>}
        {"number" in value ? <p class = "squareNumber"><b>{value["number"]}</b></p> : <p></p>}
        {value["piece"] == "" ? <img></img> : <img src = {process.env.PUBLIC_URL + '/' + value["piece"] + ".svg" }></img>}
